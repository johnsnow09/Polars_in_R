...
```{r}
data.table::fread("bse_compiled_2023_04_13.csv")
```
...
perf_7mb__duck <- microbenchmark(
               ...
               data_table = data.table::fread("bse_compiled_2023_04_13.csv"),
...
perf_test_duck_fn <- function(data_file){
  perf_data <<- microbenchmark(
               ...
               data_table = data.table::fread("bse_compiled_2023_04_13.csv") %>% head(n=10),
...
perf_test_duck_full <- function(data_file){
  perf_data <<- microbenchmark(
               ...
               data_table = data.table::fread(data_file),
...
perf_mainpulation <- microbenchmark(
          ...
          data_table_dtplyr = data.table::fread("T_ONTIME_REPORTING.csv") %>% 
                  filter(ORIGIN == "JFK") %>% 
                  group_by(ORIGIN_CITY_NAME,DEST_CITY_NAME) %>% 
                  summarise(counts = n()),
